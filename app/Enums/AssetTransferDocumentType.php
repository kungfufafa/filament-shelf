...
enum AssetTransferDocumentType: string
{
    ...
    /**
     * Resolve the document type from the transfer's sender and receiver.
     *
     * Uses the simple `role` column on the users table (replacing spatie
     * permission) to determine who is the General Affairs department.
     */
    public static function fromUsers(?User $fromUser, ?User $toUser): ?self
    {
        ...
        // Identify the main General Affairs department account (ID 2 with
        // username 'adminga' in production, or named 'GA' in tests).
        ...
        if ($fromIsGeneralAffair && $toIsGeneralAffair) {
            if ($toIsMainGA) {
                return self::PengembalianBarang;
            }

            if ($fromIsMainGA) {
                return self::SerahTerima;
            }

            return self::PengalihanBarang;
        }
