...
class AssetResource extends Resource
{
    ...
    {
        return (array) Cache::remember('asset_category_options', 300, function () {
    ...
    /**
     * Resolve the type of a custom attribute by its id.
     */
            ...
            Grid::make(2)
                ->schema([
                    ...
                    Select::make('nbh_responsible_user_id')
                        ->options(fn () => Cache::remember('user_options', 300, fn () => User::orderBy('name')->pluck('name', 'id')->toArray()))
                        ->default(fn () => auth()->id())
                        ->searchable()
                        ->required(),
                ...
                Grid::make(2)
                    ->schema([
                        Section::make('Informasi Aset')
                            ->schema([
                                ...
                                Select::make('brand_id')
                                    ->options(fn () => Cache::remember('brand_options', 300, fn () => Brand::orderBy('name')->pluck('name', 'id')->toArray()))
                                    ->searchable()
                                    ->required()
                                    ->createOptionForm([
                                        TextInput::make('name')
                        ...
                        Section::make('Kondisi & NBH')
                            ...
                            ->columns(3)
                            ->visible(fn () => auth()->user()?->hasRole('super_admin') || auth()->user()?->hasRole('general_affair')),
                ...
                Section::make('Detail Pembelian')
                    ->schema([
                        ...
                        Select::make('asset_location_id')
                            ->options(fn () => Cache::remember('asset_location_options', 300, fn () => AssetLocation::orderBy('name')->pluck('name', 'id')->toArray()))
                            ->searchable()
                            ->createOptionForm([
                                TextInput::make('name')
                                    ->required()
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                TextColumn::make('type')->sortable()->searchable()->toggleable(isToggledHiddenByDefault: true),
                TextColumn::make('serial_number')->sortable()->searchable()->toggleable(),
                TextColumn::make('imei1')->sortable()->searchable()->toggleable(isToggledHiddenByDefault: true),
                TextColumn::make('imei2')->sortable()->searchable()->toggleable(isToggledHiddenByDefault: true),
                TextColumn::make('item_price')->sortable()->money('IDR', true)->toggleable(isToggledHiddenByDefault: true),
                ...
                TextColumn::make('qty')
                    ->badge()
                    ->toggleable(isToggledHiddenByDefault: true),
                TextColumn::make('assetLocation.name')->sortable()->searchable()->toggleable(),
            ...
            ->actions([
                Action::make('completeRepair')
                    ...
                    ->color('success')
                    ->visible(fn (Asset $record): bool => (auth()->user()?->hasRole('super_admin') || auth()->user()?->hasRole('general_affair'))
    ...
    /**
     * Migrate serial/IMEI columns from the legacy asset columns into
     * AssetAttribute rows keyed by their custom_attribute_id.
     *
     * @param  Collection<int, Asset>  $records
     */
    protected static function pindahkanKeAssetAttributeBulk($records): void
    {
